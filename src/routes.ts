...
export function registerRoutes(app: Express) {
  ...
  app.post("/api/capture/:publicToken/upload", uploadRateLimit, upload.single("photo"), async (req, res) => {
    const garment = await GarmentModel.findOne({ publicToken: req.params.publicToken }).lean<Garment>();
    if (!garment) return res.status(404).json({ error: "QR_NOT_FOUND" });
    if (!req.file) return res.status(400).json({ error: "PHOTO_REQUIRED" });
    if (!isSupportedImage(req.file.buffer)) return res.status(400).json({ error: "INVALID_IMAGE" });
